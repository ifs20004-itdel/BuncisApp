...
class LoginActivity : AppCompatActivity(), AuthenticationCallback {
    ...
    private fun getVessel() {
        loginViewModel.getVessel { success, data ->
            if (success) {
    ...
    private fun setupAction() {
        binding.btnLogin.setOnClickListener {
            val bunkerCode = binding.edLoginUsername.text.toString()
            val password = binding.edLoginPassword.text.toString()
            if (bunkerCode.isEmpty() || password.isEmpty()) {
                Toast.makeText(this@LoginActivity, "Isi data yang kosong!", Toast.LENGTH_SHORT)
                    .show()
            } else {
